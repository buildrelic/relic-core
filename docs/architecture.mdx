...
│   └── src/relic/ingest/   # github, linear, mappers, sessions, raw_store, checkpoint
...
AgentSession episode (see the AgentSession amendment in
[ADR-0001](/adr/0001-engram-graph-ontology)) through the same `load_episodes` ingest uses.
The capture payload → `EpisodeSpec` mapping (transcript distillation plus the
`AgentSession` body) is `relic.ingest.session_to_episode`, the peer of the
GitHub/Linear/Granola mappers, so the composition root only wires it. Serve still carries
