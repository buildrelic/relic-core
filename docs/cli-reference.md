...
```
relic ingest --repo OWNER/NAME [--limit N] [--months M] [--bulk] [--fresh] [--no-progress]
```
...
(the per-repo checkpoint under `./data/ingest/`). See [ingestion.md](/ingestion).
...
answer? A malformed or unreadable gold set exits 1.
...
long-running command. See [skills.md](/skills).
...
nothing, never raises. See [configuration.md](/configuration).
...
|---|---|---|---|
| `ingest` | `--repo` | `--limit`, `--months`, `--bulk`, `--fresh`, `--no-progress` | yes |
