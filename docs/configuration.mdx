...
- **The episode spool.** Mapped episodes under `./data/spool/<group_id>/`, one JSON
  file per episode. Written by capture (`ingest`), read by `relic load`. It decouples
  the fast, no-LLM capture from the slow extraction. See [ingestion.md](/ingestion).
