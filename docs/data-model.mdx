...
keys mirror the flat graph attributes so the extractor maps cleanly onto them. The
flatten-and-rename the extractor performs from the body to the flat node is recorded
as executable data in
[`relic.graph.field_map`](https://github.com/buildrelic/relic-core/blob/main/packages/relic-graph/src/relic/graph/field_map.py)
(`ARTIFACT_FIELD_MAPS`) and checked by `tests/test_field_map.py`, so a node field with
no body source — or a mapped body path that no longer resolves — fails in CI instead of
surfacing as a silent null at graph-read time.
...
`additions`/`deletions`, `commits`, derived `timestamps`). The full target schema the
graph side grows into, the node and edge types with their `(source, target)` signatures
and attributes, the multi-valued rule, and the connector-to-provides map, is the
[engram graph ontology ADR](/adr/0001-engram-graph-ontology). Capture leads, extraction
follows.
</Note>
