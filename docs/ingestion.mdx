...
- **Phase timing.** Every run ends with a compact timing block on stderr: fetch,
  map + raw store, load (extraction), and setup, each with its share of the
  total, plus the per-episode load rate. It makes the LLM-bound shape of a run
  obvious at a glance: the load phase is almost the whole clock.
