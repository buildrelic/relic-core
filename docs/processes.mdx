...
| `relic ingest` | CLI | one-shot | GitHub, Linear | graph, raw store, spool | GitHub token, OpenAI, FalkorDB |
| `relic load` | CLI | one-shot | spool, graph | graph, checkpoint | OpenAI, FalkorDB |
...
- **Invocation.** `relic ingest --repo owner/name [--limit N] [--months M] [--bulk] [--fresh] [--no-progress] [--no-load]`.
...
- **Writes.** The FalkorDB graph (one episode per PR and per issue, landed under
  ...
  episode spool at `./data/spool/<group_id>/`; the checkpoint at
  `./data/ingest/<group_id>.log`.
...
- **Two halves.** A fast, deterministic capture (fetch, map, raw store, spool) with
  no LLM, then the LLM-heavy extraction into the graph. `--no-load` runs only the
...
The LLM-heavy half of ingest, run on demand. Extracts spooled episodes into the
graph.

- **Invocation.** `relic load --repo owner/name [--limit N] [--bulk] [--fresh] [--no-progress]`.
- **Reads.** The episode spool at `./data/spool/<group_id>/`, written by a prior
  `relic ingest` (or `relic ingest --no-load`); the per-repo checkpoint ledger; the
  graph.
- **Writes.** The FalkorDB graph (one episode per PR and per issue); the checkpoint
  at `./data/ingest/<group_id>.log`.
- **Depends on.** `OPENAI_API_KEY` (Graphiti extraction and embeddings) and
  FalkorDB. No GitHub token: it reads the spool, not the network.
- **Resumable.** Shares the checkpoint with `ingest`, so it skips episodes already
  loaded and a re-run only extracts what is new. An empty spool is a no-op.
- **`--limit`.** Extracts at most N not-yet-loaded episodes for case-by-case
  loading. Re-run to drain the rest. `--fresh` ignores the checkpoint and reloads
  every spooled episode.
