...
- **HTTP recall.** `serve-http` exposes recall as an HTTP API (`GET /v1/recall`)
  for the web app, returning the facts and their sources as JSON. First HTTP
  surface on relic-core. See [CLI reference](/cli-reference).
