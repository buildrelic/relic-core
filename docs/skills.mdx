...
them to coding agents. It is decoupled from the engram store: nothing here
imports ingestion or recall. The unit of currency is `SkillIR` (see
...
<Note>
Verify is the one human gate. Only verified skills are emitted to `.claude/skills/`
or exposed by `relic serve`. Drafts and deprecated skills are never served, and a
deprecated skill cannot be verified again.
</Note>
...
`verify`, `emit`, and the MCP server read from. It has no engram or network
...
- **`recall_memory`.** When a recall function is supplied, query the engram
  store and get facts with their sources.

Recall is injected, not imported. The CLI opens the engram store, wraps it in a
recall function ([`_make_recall_fn`](https://github.com/buildrelic/relic-core/blob/main/packages/relic-cli/src/relic/cli.py)), and passes it in. It is
served whenever Postgres is reachable; no model API key is involved. The recall
is scoped to `--repo` (or `TARGET_REPO`)'s scope slug, and runs workspace-wide
without one. If opening the store fails (Postgres down), `serve` logs to stderr
and runs without `recall_memory`, still serving skills. This keeps
`mcp_server.py` free of any store or network dependency, and keeps the server
useful when memory is unavailable.
...
ingestion and the engram. The slice test
...
The Phase 4 compiler is the missing piece that turns the engram into skills. The
detector ([`compile/detect.py`](https://github.com/buildrelic/relic-core/blob/main/packages/relic-engram/src/relic/compile/detect.py)) will find a
recurring procedure with a deterministic query over the engram plus a support and
confidence threshold. The compiler ([`compile/compiler.py`](https://github.com/buildrelic/relic-core/blob/main/packages/relic-engram/src/relic/compile/compiler.py))
