"""Engram: the Graphiti client factory and the graphiti-core workarounds.

Builds the configured `Graphiti` (LLM provider selection + FalkorDB driver) and applies
the graphiti-core 0.29.x monkeypatches. The flat ontology types this module used to own
now live in `relic.graph.schema`; the Memory seam (`relic.graph.memory`) will absorb this
factory next.
"""
